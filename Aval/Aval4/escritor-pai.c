...
int main(int argc, char *argv[]){
    ...
    int runningChildren;
    pid_t pid[NB_CHILDS_ES];
    char *childExecPath = FILE_PATH_ES;
    
    
    for(runningChildren=0; runningChildren<NB_CHILDS_ES; runningChildren++){
        ...
        if (pid[runningChildren] < 0) {
            ...
            if (execl(childExecPath, FILE_NAME_ES, NULL) == -1) {
